...
- 日期：2026-09-12
- 关联：连接器 [packages/connectors/](../../../packages/connectors/) ｜ 数据接入策略 [DATA_SOURCES.md](../../理论/调研/品牌/数据接入.md)
...
- **MeOS 后端需要在该时间点处于运行状态**——cron 不会自动拉起后端。如果你的 `./scripts/meos start` 是手动起的（`meos status` 可确认 api 是否在跑），要么把后端挂到 launchd 让它常驻，要么改成「上班后才打开」的频率，例如工作日 09:30（你打开 MeOS 之后再触发）。
...
| 日志里 `FAIL bilibili code=-352` 或 HTTP 412 | 投稿列表需要 cookie；按 [DATA_SOURCES.md §2](../../理论/调研/品牌/数据接入.md#1-官方开放-apioauth-授权自己的账号--首选能走则走) 取登录态 cookie |
