...
- **标准先行**：通过 [MeLog Standard](./标准.md) 开放数据格式与接口，连接器与技能可由社区共建。
...
可由本地 Skill Runner 或 MCP 工具桥接（见[标准文档 §4](./标准.md)与下方 LLM 运行器章节）。

### 标准
MeLog Standard v0.1 的摘要页（信封字段、Ingest 规范、MCP 工具、Skill 清单），
完整规范见 [docs/实践/记录/标准.md](./标准.md)。

### 口述打卡
时间线页顶部的手动录入卡片：在潮汐 / Keep 看完数据后用键盘听写念出来，
规则解析成结构化健康条目，预览确认后双写时间线与健康记录，重复提交幂等。
句式模板、解析规则与 API 见 [docs/实践/记录/口述打卡.md](./口述打卡.md)。
