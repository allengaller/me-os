...
| 维度 | 英文 | 核心问题 | 包含模块 |
|---|---|---|---|
| 方向 | Direction | 我要去哪里 | 愿景、领域、目标、关键结果、心态、平衡轮、工作流画布 |
...
在五维核心之上，MeOS 已扩展出两个增量板块（合计七个板块），另设两个聚合入口：

| | 定位 | 回答的问题 | 路由 |
|---|---|---|---|
| **MeLog**（增量板块） | 个人生活数据汇聚层与加工层 | 我的生活数据说了什么 | `/melog` |
| **品牌**（增量板块） | 所有输出的终点与放大器 | 我的输出如何影响世界 | `/brand` |
| **Today**（聚合入口） | 今日工作台 | 我今天的状态如何 | `/` |
| **Workbench**（聚合入口） | 个人工作台 | 我现在的整体状态如何 | `/workbench` |

> 工作流画布属于**方向板块内**的能力（目标之间的可视化编排），不是独立板块。
